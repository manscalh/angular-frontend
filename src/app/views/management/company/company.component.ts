...
import { PreserveSearchCompanyService } from 'src/app/core/filters/compay.filter';
...
export class CompanyComponent implements OnInit {
  ...
  constructor(
    ...
    private preserveSearch: PreserveSearchCompanyService
    ) { }
  ...
  handlerNew(){
    this.router.navigateByUrl('/management/company/details');
  }

  handlerEdit(id:number){
    this.router.navigateByUrl('/management/company/details/'+id);
  }

  handlerDelete(){
    this.modalRef?.hide();
    this.isLoadingSearch = true;
    this.CompanyService.delete(this.objDelete).subscribe(
      (res) => {
  ...
  private getAll(page: number = 1, perPage: number = 5) {
    this.isLoadingSearch = true;
    this.CompanyService.getAll(page, perPage).subscribe(
      (res) => {
  ...
  private getAllByFilter(filter: string, page: number = 1, perPage: number = 5) {
    this.isLoadingSearch = true;
    this.CompanyService.getAllByFilter(filter, page, perPage).subscribe(
      (res) => {
