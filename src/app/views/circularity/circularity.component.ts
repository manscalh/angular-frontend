...
import { PreserveSearchCircularityService } from 'src/app/core/filters/circularity.filter';
...
export class CircularityComponent implements OnInit {
  ...
  constructor(
    ...
    private preserveSearch: PreserveSearchCircularityService
    ) { }
  ...
  handlerNew(){
    this.router.navigateByUrl('/circularity/start');
  }

  handlerEdit(id:number){
    this.router.navigateByUrl('/circularity/start/'+id);
  }

  handlerDelete(){
    this.modalRef?.hide();
    this.isLoadingSearch = true;
    this.CircularityService.delete(this.objDelete).subscribe(
      (res) => {
  ...
  private getAll(page: number = 1, perPage: number = 5) {
    this.isLoadingSearch = true;
    this.CircularityService.getAll(page, perPage).subscribe(
      (res) => {
  ...
  private getAllByFilter(filter: string, page: number = 1, perPage: number = 5) {
    this.isLoadingSearch = true;
    this.CircularityService.getAllByFilter(filter, page, perPage).subscribe(
      (res) => {
